...
// std::uniform_real_distribution<double> distribution(-1.0, 1.0); // 0~-1
...
int main(int argc, char **argv)
{
    ...
    std::vector<double> _xarm_joint_angle;
    ...
    while (rclcpp::ok())
    {
        ...
        std::uniform_real_distribution<double> _joint_angle_A(-M_PI, M_PI);
        std::uniform_real_distribution<double> _joint_angle_B(-(118 / 180.0) * M_PI, (120 / 180.0) * M_PI);
        std::uniform_real_distribution<double> _joint_angle_C(-(225 / 180.0) * M_PI, (11 / 180.0) * M_PI);
        std::uniform_real_distribution<double> _joint_angle_D(-M_PI, M_PI);
        std::uniform_real_distribution<double> _joint_angle_E(-(40 / 180.0) * M_PI, (150 / 180.0) * M_PI);
        std::uniform_real_distribution<double> _joint_angle_F(-M_PI, M_PI);
        _xarm_joint_angle = {_joint_angle_A(engine),
                             _joint_angle_B(engine),
                             _joint_angle_C(engine),
                             _joint_angle_D(engine),
                             _joint_angle_E(engine),
                             _joint_angle_F(engine)};


        joint_plan_req->target = _xarm_joint_angle;
        call_request(joint_plan_client_, joint_plan_req);
        call_request(exec_plan_client_, exec_plan_req);
        RCLCPP_INFO(node->get_logger(), "move to _xarm_joint_angle");
